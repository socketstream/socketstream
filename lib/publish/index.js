    ...
    api: function(transport) {
      ...
      methods = {
        all: function() {
          ...
          transport.send(obj);
          if (!isInternal(event)) {
            return ss.log.info('➙'.cyan, 'event:all'.grey, event);
          }
        },
        socketId: function() {
          ...
          transport.send(obj);
          return ss.log.info('➙'.cyan, ("event:socketId:" + socketId).grey, event);
        },
        users: function() {
          ...
          transport.send(obj);
          return ss.log.info('➙'.cyan, ("event:users:[" + (users.join(',')) + "]").grey, event);
        },
        channels: function() {
          ...
          transport.send(obj);
          return ss.log.info('➙'.cyan, ("event:channels:[" + (channels.join(',')) + "]").grey, event);
        }
      };
