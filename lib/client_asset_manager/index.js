var codeWrappers, formattersByExtension, http, log, packAssets, res;
...
exports.init = function(root, router) {
  var Client, clients, formatters, ssClient;
  ...
  return {
    ...
    load: function(client) {
      ...
      if (packAssets) {
        ...
        for (name in clients) {
          client = clients[name];
          _results.push(client.pack(ssClient, formattersByExtension));
        }
        return _results;
      } else {
        asset = require('./asset').init(root, formattersByExtension, codeWrappers);
        return require('./serve_live').init(router, ssClient, asset);
      }
    }
  };
};
