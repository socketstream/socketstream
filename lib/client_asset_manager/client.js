    ...
    Client.prototype.html = function(ssClient, formatters, packAssets, cb) {
      ...
      if (ssClient.html != null) {
        return ssClient.html(function(codeForView) {
          var ext, f, files, htmlFormatters;
          ...
          if (files && files.length > 0) {
            htmlFormatters = (function() {
              var _results;
              _results = [];
              for (ext in formatters) {
                f = formatters[ext];
                if (f.assetType === 'html') _results.push(f);
              }
              return _results;
            })();
            return templateEngine.generate(root, templateDir, files, htmlFormatters, function(templateHTML) {
