...
function loadStaticDirs(staticPath,assetsPath) {
  ...
  if (fs.existsSync(staticPath)) {
    ...
    staticDirs = fs.readdirSync(staticPath);
...
module.exports = function(root) {
  return {
    ...
    load: function(staticPath, assetsPath, sessionStore, sessionOptions) {
      ...
      assetsPath = pathlib.join(root, assetsPath);
      loadStaticDirs(staticPath,assetsPath);
      ...
      app.use(connect.favicon(staticPath + '/favicon.ico'))
   ...
      /* Append any custom-defined middleware (e.g. everyauth) */
      ...
      // serve assets and static out of separate paths.
      app.use(serveStatic('/assets',assetsPath, settings['static']));
      app.use(serveStatic(null,staticPath, settings['static']));

      /* Allow for the use of staticCache, if the option is passed */
      if (settings.staticCache) {
        app.use(connect.staticCache(settings['staticCache']));
      }
