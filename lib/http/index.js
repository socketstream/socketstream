...
module.exports = function(ss) {
  return {
     ...
     */
    load: function(staticPath, assetsPath) {
      if (!app) {
        // if ss.http.middleware isn't used before server start, no need to load it
        return;
      }

      staticPath = path.join(ss.root, staticPath);
      assetsPath = path.join(ss.root, assetsPath);
      ...
      if (ss.session.strategy.sessionMiddleware) {
        if (ss.session.strategy.sessionMiddleware.forEach) {
          ss.session.strategy.sessionMiddleware.forEach(function(middleware) {
            app.use(middleware);
          });
        } else {
          app.use(ss.session.strategy.sessionMiddleware);
        }
      }
