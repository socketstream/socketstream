...
// Request Handlers
var request = exports.request = require('./request/index').init(root, api);

// Websocket Layer (transport, message responders, publish)
var ws = exports.ws = require('./websocket/index').init(root, request, api);
...
exports.start = function(httpServer) {
  ...
  var server = {
    responders:      request.responders.load(),
  ...
  if (httpServer) {
    ...
    var ssClient = require('./browser_client/index').init(wsTransport, server.responders);

    // Load Client Asset Manager
    client.load(ssClient, api);
