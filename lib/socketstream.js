...
function start(httpServer) {
  ...
  if (typeof httpServer === 'string' || httpServer instanceof Array) {
    httpServer = null;
  } else {
    tasks.unshift();
  }
  ...
  if (!api.orchestrator.hasTask('default')) {
    var defaultTasks = [];
    if (client.options.packedAssets) {
      defaultTasks.push(client.options.packedAssets.all? 'pack-all':'pack-if-needed');
    } else {
      defaultTasks.push('live-assets');
    }
    if (client.options.liveReload) {
      defaultTasks.push('live-reload');
    }
    // if (httpServer)
    defaultTasks.push('serve');

    api.orchestrator.add('default',defaultTasks);
  }

  // Load Client Asset Manager
  client.load();

  // api.orchestrator.on('task_start',function(ev) {
  //   console.log('start task', ev);
  // });
  ...
  api.orchestrator.start(tasks, done); 
