...
try {
  ...
  // Ugly, but this doing it this way instead of using path.exists() means it will look for db.coffee, db.js, or anything else just fine
  if (!e.message.match(/Cannot find[\s\S]*config\/db/)) throw(e)
}
