...
module.exports = function(ss, router, options, orchestrator) {
  ...
  return function() {
    ...
    ss.defaultTask('start-server',['load-api'],function(done) {
      ss.ws.listen(ss.http.settings.port || 3000, function() {
    ...
    var liveAssetsDeps = (!options.servePacked && !options.packedAssets)? ['start-server'] : []
    ss.defaultTask('live-assets', liveAssetsDeps, function() {
