  ...
  return {
    ...
    packAssets: function(opts) {
      ...
      if (options.packedAssets && process.listeners('uncaughtException').length === 0) {
        return process.on('uncaughtException', function(err) {
          ss.log.error('Uncaught Exception!'.red);
          return ss.log.error(err.stack);
        });
      }
    },
    ...
    load: function() {
      ...
      if (options.packedAssets) {
        ...
        if (!packAssets) {
          ss.log.info('i'.green, "Attempting to find pre-packed assets... (force repack with SS_PACK=1)".grey);
          for (name in clients) {
            client = clients[name];
            if (id = options.packedAssets.id || determineLatestId(client)) {
              client.id = id;
              ss.log.info('✓'.green, ("Serving client '" + client.name + "' using pre-packed assets (ID " + client.id + ")").grey);
            } else {
              ss.log.info('!'.red, ("Unable to find pre-packed assets for '" + client.name + "'. All assets will be repacked").grey);
              packAssets = true;
            }
          }
        }
