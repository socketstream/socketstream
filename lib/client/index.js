...
module.exports = function(ss, router) {
  ...
  function serveOndemand() {
    require('./serve/ondemand')(ss, router, options);
  }

  ss.orchestrator.add('serve-dev', function() {
  ...
  return {
    ...
    load: function(done) {
      ...
      ss.orchestrator.start('serve',done); // this will be moved to socketstream.js
    },

    addTasks: function() {

      ss.orchestrator.add('pack-all', this._packTasks());
      ss.orchestrator.add('pack-report', this._packReport.bind(this));
      ss.orchestrator.add('serve', this._serveTasks(), serveOndemand);
    ...
    _packReport: function() {
      //TODO print pack report for asset packing plan
      log.info('i'.green, 'Attempting to find pre-packed assets... (force repack with SS_PACK=1)'.grey);
      ss.bundler.forEach(function(bundler) {
        if (bundler.servingAssetId) {
      ...
      if (options.packedAssets) {
        ss.bundler.forEach(function(bundler){
          ...
          if (id) {
            ...
            bundler.client.servingAssetId = id;
          } else {
    ...
    _serveTasks: function() {
      if (options.packedAssets) {
        return ['pack-all'];
      } else {
        return options.liveReload? ['serve-dev','live-reload']:['serve-dev'];
      }
    },
