...
module.exports = function(ss, router) {
  ...
  orchestrator.add('serve-dev', function() {
    require('./serve/dev')(ss, router, options);
  });

  orchestrator.add('live-reload', function() {
  ...
  return {
     ...
     * In the future this may migrate to socketstream proper.
     *
    ...
    load: function(done) {
      ...
      this.addTasks();
      this.orchestrator.start('serve',done); // this will be moved to socketstream.js
    },

    addTasks: function() {

      this.orchestrator.add('build', this.buildTasks());
      this.orchestrator.add('serve', this.serveTasks(), serveOndemand);
      ...
      ss.bundler.forEach(function(bundler) {
        ...
        // Pack Assets
        orchestrator.add(bundler.client.name+':pack',function() {
    ...
    serveTasks: function() {
      if (options.packedAssets) {
        ...
        return options.liveReload? ['build','serve-dev','live-reload'] : ['build','serve-dev'];
      }
    },
