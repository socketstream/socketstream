...
//TODO will be a module injected by socketstream-...-session
// perhaps it will be a client decorator function with ss passed
exports.sendAuth = function(socket) {
  //sock.secure = secure channel ?
  //sock.id = session id

  var sessionToken = require('socketstream-session').sessionToken;
  if (sessionToken) {
    socket.send('X|' + sessionToken); // sending access claims
  } else{
