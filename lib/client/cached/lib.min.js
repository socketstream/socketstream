...
Array.prototype.include=function(value){return this.indexOf(value)!=-1},Array.prototype.any=function(){return this.length>0},window.getCookie=function(c_name){if(document.cookie.length>0){c_start=document.cookie.indexOf(c_name+"=");if(c_start!=-1){c_start=c_start+c_name.length+1,c_end=document.cookie.indexOf(";",c_start),c_end==-1&&(c_end=document.cookie.length);return unescape(document.cookie.substring(c_start,c_end))}}return""},window.setCookie=function(c_name,value,expiredays){var exdate=new Date;exdate.setDate(exdate.getDate()+expiredays),document.cookie=c_name+"="+escape(value)+(expiredays==null?"":";expires="+exdate.toUTCString())};(function(){var RTM,Request,System,error,log,send,validLevel;window.exports={},window.$SS={models:{},internal:{cb_stack:{}},config:{log:{level:0}}},$SS.events={_events:{},on:function(name,funct){this._events[name]==null&&(this._events[name]=[]);return this._events[name].push(funct)},emit:function(name,params){var event,_i,_len,_ref,_results;_ref=this._events[name],_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++)event=_ref[_i],_results.push(event(params));return _results}},$SS.socket=new io.Socket(document.location.hostname,{rememberTransport:!1,port:document.location.port,secure:document.location.protocol==="https:",transports:["websocket","flashsocket"],tryTransportsOnConnectTimeout:!1}),$SS.socket.on("message",function(raw){var data;data=JSON.parse(raw),data.type||(data.type="event");return Request[data.type](data)}),$SS.socket.on("disconnect",function(){var attemptReconnection;attemptReconnection=function(){if(!$SS.socket.connected){$SS.socket.connect();return setTimeout(arguments.callee,50)}};return attemptReconnection()}),$SS.socket.connect(),window.remote=function(){var args,cb,method,options,params;args=arguments,method=args[0],$SS.config.remote_prefix&&(method=""+$SS.config.remote_prefix+"."+method),cb=args[args.length-1],params=args.length>=3?args[1]:null,options=args.length>=4?args[2]:null,cb.options=options,validLevel(4)&&(!options||!options.silent)&&console.log("<- "+method);return send({method:method,params:params,callee:method,options:options},cb)},RTM=function(){function RTM(){}RTM.prototype.findById=function(id,cb){return this._send("findById",id,cb)},RTM.prototype.find=function(){var arg,args,cb,_i,_len;args=[];for(_i=0,_len=arguments.length;_i<_len;_i++)arg=arguments[_i],args.push(arg);cb=args.pop();return this._send("find",args,cb)},RTM.prototype._send=function(action,params,cb){log(2,"<~ "+this.name+"."+action+"("+params+")");return send({rtm:this.name,action:action,params:params},cb)};return RTM}(),System={ready:function(){return $SS.socket.ready=!0},setSession:function(id){setCookie("session_id",id);return log(2,"-> Started new session: "+id)},setConfig:function(client_config){return $SS.config=client_config||{}},setModels:function(model_names){var name,_i,_len,_results;_results=[];for(_i=0,_len=model_names.length;_i<_len;_i++)name=model_names[_i],$SS.models[name]=new RTM,_results.push($SS.models[name].name=name);return _results},error:function(details){return error("SocketStream Application Error: "+details[1])}},Request={system:function(data){return System[data.method](data.params)},callback:function(data){var cb,silent;cb=$SS.internal.cb_stack[data.cb_id],silent=cb.options&&cb.options.silent,log(2,"-> "+data.callee),data.params&&validLevel(3)&&!silent&&console.log(data.params),cb.funkt(data.params);return delete $SS.internal.cb_stack[data.cb_id]},event:function(data){validLevel(2)&&console.log("=> "+data.event);return $SS.events.emit(data.event,data.params)},rtm:function(data){var cb;cb=$SS.internal.cb_stack[data.cb_id],log(2,"~> "+cb.msg.rtm+"."+cb.msg.action+"("+cb.msg.params+")"),cb.funkt(data.data);return delete $SS.internal.cb_stack[data.cb_id]}},send=function(msg,cb){var args,cb_id,recursive,retry_ms;args=arguments;try{if($SS.socket.connected===!1&&$SS.socket.connecting===!1){$SS.socket.ready=!1,$SS.socket.connect();throw"NOT_READY"}if($SS.socket.ready===!0)cb_id=Math.random().toString().split(".")[1],window.$SS.internal.cb_stack[cb_id]={funkt:cb,msg:msg},msg.cb_id=cb_id,msg=JSON.stringify(msg),$SS.socket.send(msg);else throw"NOT_READY"}catch(e){if(e==="NOT_READY")retry_ms=50,console.log("Server not ready. Waiting for "+retry_ms+"ms and retrying..."),recursive=function(){return send.apply(this,args)},setTimeout(recursive,retry_ms);else throw e}return},log=function(level,msg){if(validLevel(level))return console.log(msg)},error=function(e){if(validLevel(1))return console.error(e)},validLevel=function(level){return $SS.config.log.level>=level}}).call(this)