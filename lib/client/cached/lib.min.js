...
Array.prototype.include=function(value){return this.indexOf(value)!=-1},Array.prototype.any=function(){return this.length>0},window.getCookie=function(c_name){if(document.cookie.length>0){c_start=document.cookie.indexOf(c_name+"=");if(c_start!=-1){c_start=c_start+c_name.length+1,c_end=document.cookie.indexOf(";",c_start),c_end==-1&&(c_end=document.cookie.length);return unescape(document.cookie.substring(c_start,c_end))}}return""},window.setCookie=function(c_name,value,expiredays){var exdate=new Date;exdate.setDate(exdate.getDate()+expiredays),document.cookie=c_name+"="+escape(value)+(expiredays==null?"":";expires="+exdate.toUTCString())};(function(){var RTM,Request,System,error,log,send,sendHeartbeat,setupAPI,validLevel;window.exports={},window.SS={env:null,server:{},shared:{},models:{},internal:{cb_stack:{}},config:{log:{level:0}}},window.$SS=window.SS,SS.events={_events:{},on:function(name,funct){this._events[name]==null&&(this._events[name]=[]);return this._events[name].push(funct)},emit:function(name,params){var event,_i,_len,_ref,_results;_ref=this._events[name],_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++)event=_ref[_i],_results.push(event(params));return _results}},SS.socket=new io.Socket(document.location.hostname,{rememberTransport:!1,port:document.location.port,secure:document.location.protocol==="https:",transports:["websocket","flashsocket"],tryTransportsOnConnectTimeout:!1}),SS.socket.on("message",function(raw){var data;data=JSON.parse(raw);return data.type?Request[data.type]!=null?Request[data.type](data):console.error("Error: Unable to find a message handler for '"+data.type+"' requests! Dropping message"):console.error("Error: No message type specified. Dropping message")}),SS.socket.on("disconnect",function(){var attemptReconnection;attemptReconnection=function(){SS.socket.connecting||SS.socket.connect();return setTimeout(arguments.callee,100)};return attemptReconnection()}),SS.socket.connect(),window.remote=function(){var args,cb,msg;args=arguments,msg={type:"server"},msg.method=args[0],SS.config.remote_prefix&&(msg.method=""+SS.config.remote_prefix+"."+msg.method),msg.params=args.length>=3?args[1]:null,msg.options=args.length>=4?args[2]:null,cb=args[args.length-1],cb.options=msg.options,validLevel(4)&&(!msg.options||!msg.options.silent)&&console.log("<- "+msg.method);return send(msg,cb)},RTM=function(){function RTM(){}RTM.prototype.findById=function(id,cb){return this._send("findById",id,cb)},RTM.prototype.find=function(){var arg,args,cb,_i,_len;args=[];for(_i=0,_len=arguments.length;_i<_len;_i++)arg=arguments[_i],args.push(arg);cb=args.pop();return this._send("find",args,cb)},RTM.prototype._send=function(action,params,cb){log(2,"<~ "+this.name+"."+action);return send({type:"rtm",rtm:this.name,action:action,params:params},cb)};return RTM}(),System={init:function(data){var name,_i,_len,_ref;console.log(data),SS.env=data.env,SS.config=data.config||{},setCookie("session_id",data.session_id),_ref=data.api.models;for(_i=0,_len=_ref.length;_i<_len;_i++)name=_ref[_i],SS.models[name]=new RTM,SS.models[name].name=name;eval("SS.server = "+data.api.server),setupAPI(SS.server,[]),SS.socket.ready=!0,$?$(document).ready(function(){return app.init()}):app.init();if(data.heartbeat)return sendHeartbeat()},error:function(details){return error("SocketStream Application Error: "+details[1])}},Request={system:function(data){return System[data.method](data.params)},server:function(data){var cb,silent;cb=SS.internal.cb_stack[data.cb_id],silent=cb.msg.options&&cb.msg.options.silent,silent||log(2,"-> "+cb.msg.method,data.params),cb.funkt(data.params);return delete SS.internal.cb_stack[data.cb_id]},event:function(data){log(2,"=> "+data.event);return SS.events.emit(data.event,data.params)},rtm:function(data){var cb;cb=SS.internal.cb_stack[data.cb_id],log(2,"~> "+cb.msg.rtm+"."+cb.msg.action),cb.funkt(data.data);return delete SS.internal.cb_stack[data.cb_id]}},window.define=function(name,fn){window.defs||(window.defs={});return window.defs[name]=fn},window.require=function(name){var fn,modules;if(modules&&modules.hasOwnProperty(name))return modules[name];if(window.defs&&window.defs.hasOwnProperty(name)){modules||(modules={}),fn=window.defs[name],window.defs[name]=function(){throw new Error("Circular Dependency")};return modules[name]=fn()}throw new Error("Module not found: "+name)},send=function(msg,cb){var args,cb_id,recursive,retry_ms;args=arguments;try{if(SS.socket.connected===!1&&SS.socket.connecting===!1){SS.socket.ready=!1,SS.socket.connect();throw"NOT_READY"}if(SS.socket.ready===!0)cb_id=Math.random().toString().split(".")[1],window.SS.internal.cb_stack[cb_id]={funkt:cb,msg:msg},msg.cb_id=cb_id,msg=JSON.stringify(msg),SS.socket.send(msg);else throw"NOT_READY"}catch(e){if(e==="NOT_READY")retry_ms=50,recursive=function(){return send.apply(this,args)},setTimeout(recursive,retry_ms);else throw e}return},log=function(level,msg,params){var o;if(validLevel(level)){o=[msg],params&&validLevel(3)&&o.push(params);return console.log.apply(console,o)}},error=function(e){if(validLevel(1))return console.error(e)},validLevel=function(level){return SS.config.log.level>=level},setupAPI=function(root,ary){var key,ns,value,_results;_results=[];for(key in root)value=root[key],ns=ary.slice(0),ns.push(key),_results.push(typeof value=="object"?setupAPI(root[key],ns):root[key]=new Function('remote.apply(window, ["'+ns.join(".")+'"].concat(Array.prototype.slice.call(arguments, 0)))'));return _results},sendHeartbeat=function(){send({type:"heartbeat"},function(){});return setTimeout(arguments.callee,SS.config.heartbeat_interval*1e3)}}).call(this)