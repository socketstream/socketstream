...
module.exports = function(ss,client,options){
  ...
  function assetJS(path, opts, cb) {
    return ss.bundler.loadFile({file:path, ext:'js'}, opts, null, function(output) {
      ...
      return cb('// couldn\'t format ' + path + '\n'+err.message);
    });
  }
  ...
  function assetCSS(path, opts, cb) {
    return client.includes.css? ss.bundler.loadFile({file:path, ext:'css'}, opts, null, cb,
  ...
  function assetHTML(path, opts, cb) {
    return ss.bundler.loadFile({file:path, ext:'html'}, opts, null, function(html) {
