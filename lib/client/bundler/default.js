    ...
    ss.bundler.loadFile(entry, opts, null,
      function(output) {
        switch(entry.ext) {
          ...
          case 'css':
            return client.includes.css?
              (ss.bundler.loadFile({file:path, ext:'css'}, opts, null,
                cb,
                function(err) { return cb('/* couldn\'t format ' + path + '\n'+err.message+' */'); })) : cb(null);
        }
        switch(entry.type) {
          case 'worker':
            //TODO
      ...
      function(err) {
        switch(entry.ext) {
          case 'html':
            return cb('Couldn\'t format ' + entry.file + '');
          case 'css':
            return cb('/* couldn\'t format ' + entry.file + '\n'+err.message+' */');
          default:
            return cb('// couldn\'t format ' + entry.file + '\n'+err.message);
