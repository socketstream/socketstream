...
module.exports = function(ss,options) {
  ...
  return {
    ...
    format: function(entry, options, formatter, cb, errCb) {
      ...
      var f;
      //formatter = formatter || ( (f = ) && f.assetType === 'html' && f );

      var fullPath = ss.bundler.clientFilePath(entry);

      //src extension
      //dest extension

      // default to 'HTML' (echo/bypass)
      formatter = formatter || ss.client.formatters[extension] || ss.client.formatters['html'];

    },

    loadFile: function loadFile(entry, opts, formatter, cb, errCb) {
      var fileName = entry.file, type = entry.assetType || entry.bundle;
      var dir = path.join(ss.root, options.dirs.client);
      var p = path.join(dir, fileName);
      //TODO extension determined in view.js and pack.js
      ...
      var fullPath = this.clientFilePath(entry.file);

      // Use the formatter to pre-process the template before bundling
      //TODO format with bundler
      try {
        //TODO windows path support ... p.replace(/\\/g, '/')
        return formatter.call(fullPath, opts, function(output) {
          return cb(output);
        }, function(err) {
          return errCb(err);
        });
    ...
    packAssetSet: function packAssetSet(assetType, client, postProcess) {
      ...
      function processFiles(fileContents, i) {
        ...
        return bundler.asset({ext:'', bundle:assetType, file:file}, {
