...
module.exports = function(ss,options) {
  ...
  return {
    ...
    format: function(entry, options, formatter, cb, errCb) {
      ...
      var f;
      formatter = formatter || ( (f = ss.client.formatters[extension]) && f.assetType === 'html' && f );
      ...
      try {
        //TODO locals constants
        return formatter.call(fullPath, {}, function(output) {
          return cb(output);
        }, function(err) {
          return errCb(err);
        });
    ...
    loadFile: function loadFile(entry, opts, formatter, cb, errCb) {
      var fileName = entry.file, type = entry.ext;
      ...
      return formatter.call(p.replace(/\\/g, '/'), opts, cb, errCb);
    },
    ...
    packAssetSet: function packAssetSet(assetType, client, postProcess) {
      ...
      function processFiles(fileContents, i) {
        ...
        return bundler.asset({ext:assetType, file:file}, {
    ...
    entries: function entries(client, assetType) {
      ...
      function pushMainEntries(from) {
        return magicPath.files(path.join(ss.root, options.dirs.client), from).forEach(function(file) {
          return _entries.push({file:file,importedBy:from,includeType:includeType,ext:ext});
        });
      }
