  ...
  return {
    ...
    sourcePaths: function(paths) {
      ...
      function relativePath(p, dirType) {
          ...
          return relativeStart? path.join(options.dirs.client.replace(/^\//,''),p) : path.join(options.dirs[dirType].replace(/^\//,''), p);
      }
    ...
    entries: function entries(client, assetType) {
        ...
        case 'tmpl':
          ...
          client.paths.tmpl.forEach(function(tmpl) {
            if (tmpl.substring(tmpl.length-2) === '/*') {
              var matching = magicPath.files(path.join(ss.root,tmpl.substring(0,tmpl.length-2)), '*');
      ...
      function pushMainEntries(from) {
        //TODO windows sep replace of from
        return magicPath.files(ss.root, from).forEach(function(file) {
