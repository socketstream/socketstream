...
module.exports = function(ss,options) {
  ...
  return {
    ...
    format: function(entry, options, formatter, cb) {
      ...
      var fullPath = bundler.clientFilePath(entry);
      ...
      try {
        ...
        return formatter.call(fullPath, {}, function(err, output) {
          return cb(output);
        });
      } catch (e) {
        ...
        case 'tmpl':
          client.paths.tmpl.forEach(function(tmpl) {
            if (tmpl.substring(tmpl.length-2) === '/*') {
              ...
              _entries = _entries.concat(matching.map(function(p) { return path.join(tmpl.substring(0,tmpl.length-2),p); }));
            }
            //TODO entry format
            else {
