...
module.exports = function(ss,options) {
  ...
  return {
    ...
    packAssetSet: function packAssetSet(assetType, client, postProcess) {
      ...
      async.mapSeries(bundler.entries(assetType,system.assets), iterator, callback);
      ...
      function iterator(entry, callback) {
        ...
        if (entry.content) {
          ...
          bundler.asset(entry/*bundler.entryFor(assetType, entry.file)*/, options, function(output) {
      ...
      function callback(err, results) {
        ...
        fs.writeFileSync(fileName, postProcess(results));
