...
var cleanCSS, deleteOldFiles, existslib, formatKb, fs, mkdir, pathlib, system, view;
...
module.exports = function(ss, client, options) {
  var asset, clientDir, containerDir, packAssetSet, magicPath;
  magicPath = require('./magic_path')(ss);
  ...
  packAssetSet = function(assetType, paths, dir, postProcess) {
    ...
    writeFile = function(fileContents) {
      ...
      return ss.log.info('✓'.green, 'Packed ' + filePaths.length + ' files into ' + fileName.substr(ss.root.length));
    };
  ...
  packAssetSet('css', client.paths.css, options.dirs.css, function(files) {
    ...
    ss.log.info(("  Minified CSS from " + (formatKb(original.length)) + " to " + (formatKb(minified.length))).grey);
    return minified;
  });
  ...
  return view(ss, client, options, function(html) {
    ...
    return ss.log.info('✓'.green, 'Created and cached HTML file ' + fileName.substr(ss.root.length));
  });
};
