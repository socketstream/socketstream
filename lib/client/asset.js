...
wrapCode = function(code, path, pathPrefix, options) {
  ...
  if (__indexOf.call(pathAry, 'entry.js') === -1 && options && options.brosefyExcludePaths) {
    for(i in options.brosefyExcludePaths) {
      if (options.brosefyExcludePaths.hasOwnProperty(i)) {
        if ( path.split( options.brosefyExcludePaths[i] )[0] === "" ) {
          console.log(i, options.brosefyExcludePaths[i], path);
          return code;
        }
      }
    }
