...
processSession = function(socket) {
  var cookie, rawCookie, sessionId, unsignedSessionId, key;
  if (socket.sessionId) {
    return true;
  }
  try {
    (ss.session.options.key === null) ? key = "connect.sid" : key = ss.session.options.key;
    rawCookie = socket.handshake.headers.cookie;
    cookie = qs.parse(rawCookie, '; ');
    sessionId = cookie[key].split('.')[0];
