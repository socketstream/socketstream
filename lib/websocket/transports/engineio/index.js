...
var processSession = function(socket) {
  ...
  try {
    socket.sessionId= socket.transport.sid;
    return true;
  }
  catch(e) {
...
module.exports = function(ss, messageEmitter, httpServer, config){
  ...
  ss.client.send('lib', 'engine.io-client', engineioClient, {minified: true});
  ...
  ws.on('connection', function(socket) {
    ...
    //console.log("process.request.connection", socket.request.connection.remoteAddress);
    ...
    //console.log('socket.sessionId, socket.id', socket.sessionId, socket.id);
    ...
    event: function() {
      ...
      return {
        ...
        all: function(msg) {
          ...
          //console.log('openSocketsById', openSocketsById);
          for (var id in openSocketsById) {
            console.log('id', id);
            if (openSocketsById.hasOwnProperty(id)) {
              //console.log(openSocketsById[id].send);
