...
var processSession = function(socket) {
  try {
    var rawCookie, cookie, sessionId, unsignedSessionId;
    if(socket.upgraded) {
      // this line for diagnostics is getting nothing
      //console.log("we have a upgrade req with",socket.request.headers.upgradeReq);
      // so I'm thinking either missing something need to implement or just can 
      // clear this case out.  Will be doing a lot more testing and devvs this week (29.1.2015)
      // in engine.io both on SocketStream and other apps--> will inform upcoming devs
      rawCookie = socket.request.headers.upgradeReq.cookie;
    }
    else {
