...
var processSession = function(socket) {
  try {
    var cookie_list = qs.parse(socket.request.headers.cookie, ';');
    //c('cookie_list', cookie_list);
    var cursor = cookie_parser.signedCookie(cookie_list[' connect.sid'], "SocketStream");
    //c('cursor', cursor);
    socket.sessionId = cursor;
    return true;
  }
  catch(e) {
