  ...
  "pages": [
    {
      ...
      "keywords": "__tip _latest ability aborted accelerating access achieved action actions active add-on addition additional addy ajax alan alert allow allows alternatives amazing analysing analytics angular answer api apis app appear application applications appreciated approaches apps areas argument ashkenas asset assets assumption asynchronous authenticate authentication authors auto-restart automatically backbone backend baked banner barnes basic basics bazon best best-of-breed bi-directional big bloating blog box broken browser browsers bug build building bundled business call called calling care catered cd cdn cdns change changelog changes changing channels chat choice choosing chrome client client-side clients cloud clunky code codebase coffeekup coffeescript combination coming command commands commercial commit common communication community compatible complexity comprehensive compressed config configure connect connection connections connects consider console constantly contact content-rich continue contributed contributions contributors convenience convention core couchdb craig create created creator credits css currently custom dahl dashboard data database db decided default define demand demo demos dependencies dependent depending deploy deployment derbyjs derived designing details developer developers developing development differently directory display distributed documentation documented download driver eagle-eyed eases easily easy ec2 effortless ember empty enable enabling encounter end-to-end engine engines enhance established events exactly example examples excellent exceptions expanding experiment exports extending facebook fail failed fallback fallbacks faqs fast feature featured features feedback file find fine firefox fit fixes flexible focus follow format formats formatters forthcoming framework frameworks francisco frees front fully function future game games gazillion generate git github google great group grow grunt grunt-cli guide guillermo hall hand handlers handling hard header help helping heroku high-speed history hogan holowaychuk honouring hook hook-in hookup hope host htm html html5 htmlfile http ideal ideally ideas identity image improve improvements included includes including incoming increase indicates infoq initial inspiration install installed installing installs instantly instructions integrate integrated integrating intend interested internal introduce introduction invoke invoked invoking io ipads iphones isaac jade javascript jensen jeremy jordan jquery js jshint json-over-rpc jsonp-polling keen key kind krtconf latency lead level lib libraries license licensed lint linted linting lisbon list live ll local localhost location log logged logging logic logo london longer lot low-level maintained major making managed manager manner maximize md memory mess message messages messy meteor middleware mihai milford minkin mit mobile mock mode model models modern modular module modules mongodb monitoring months moving muir multiplayer multiple myoptionalpass naming native net node nodejitsu nodemon nodemonignore non-trivial notice november npm number october offer older online opinionated optimal optimization optional optionally options__ order org organize organizing original osmani overhead overview owen packages packing pain parts pass passionate paul perfect perfectly persistent personal pick pipe place plan platform plugin polling poor popup port portland practices pre-compiled preserve private problems process processed processor production productivity project properly protocols providing pub publicly publish pubsub push pusher pushing quality question rauch re-established reactive ready real realtime realtimeconf received recommended redis reduce release released reload reloads repl request requests require requires res resize responder responders response responsive restart retrieval return returned rich robert roman routed router routing rpc running ryan sadly safari sake sales salvatore san sanfilippo save saved scalable scaling schlueter seamless seamlessly search second seconds send sends sensible seo september serialization serve server server-side servers session sessions set settings share side simple simply simulating single single-page site slow social socket socketio socketstream solution sourceforge spaghetti special speed square squared squares ss start started starting static sticky stock storage store stored streaming stylus sub-directories substack sudo support supported supports switch synching syncing synonymous task tasks tastes template templates terminal test testing tests things third-party time tj tools trading transpilers transport transports true tutorials twitter type ui undefined unique update urls use-case user users validation vanilla var ve versions videos view visit visits walks warning warnings ways web website websocket websockets well-maintained whirl windows wondering work workers working works wrapper wrappers write writing written ws xhr-polling youtube zepto zeromq"
    },
    {
      "section": "tutorials",
      "id": "authentication",
